...
async def run_tasks_full(ctx: ExecutionContext, p: PipelineSnapshot) -> tuple[int, int]:
    if not p.tasks:
    ...
    if not is_allowed_target(final_target):
        raise ValueError(f"Task-level target_table" f" is not allowed: {final_target!r}")
